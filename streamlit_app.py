...
if final_image is not None:
    ...
    if not models_loaded:
        ...
        try:
            ...
            # 3. نظام الفحص المتقاطع الذكي (المعايرة الرقمية الدقيقة بناءً على تقرير الكولاب)
            if detected_plant == "ريحان":
                # رادار الريحان: لو موديل الأمراض لقط مرض ريحان، يبقى مصاب فوراً
                if "Basil" in detected_disease:
                    is_healthy = False
                else:
                    # تعديل الكولاب: الريحان السليم دايماً قيمته متقزمة وقريبة من الصفر وتحت الـ 0.45
                    is_healthy = health_value <= 0.45
            else:
                # رادار النعناع: النعناع السليم أثبت قوته وتمركزه الآمن ابتداءً من 0.75 فأعلى
