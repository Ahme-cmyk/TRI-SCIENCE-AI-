...
if final_image is not None:
    ...
    if not models_loaded:
        ...
        try:
            ...
            # 3. نظام الفحص المتقاطع الذكي (المعايرة الرقمية النهائية المستنتجة من حصر الداتا)
            if detected_plant == "ريحان":
                # رادار الريحان: لو موديل الأمراض لقط مرض ريحان، يبقى مصاب فوراً
                if "Basil" in detected_disease:
                    is_healthy = False
                else:
                    # بناءً على الحصر: الريحان السليم متوسطه 0.28 والمريض يعلو فوق 0.41
                    # العتبة الدقيقة الفاصلة هي 0.35 لمنع التداخل تماماً
                    is_healthy = health_value <= 0.35
            else:
                # رادار النعناع: النعناع السليم متوسطه 0.80 والمريض يهبط إلى 0.65
                # العتبة الدقيقة الفاصلة هي 0.72 لضمان أعلى استقرار
